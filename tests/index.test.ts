...
import { APIPromise } from 'multazim/core/api-promise';

import util from 'node:util';
import Multazim from 'multazim';
import { APIUserAbortError } from 'multazim';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['MULTAZIM_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Multazim) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new Multazim({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['MULTAZIM_LOG'] = 'debug';
    const client = new Multazim({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['MULTAZIM_LOG'] = 'not a log level';
    const client = new Multazim({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'MULTAZIM_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Multazim({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['MULTAZIM_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Multazim({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MULTAZIM_BASE_URL'] = 'https://example.com/from_env';
      const client = new Multazim({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['MULTAZIM_BASE_URL'] = ''; // empty
      const client = new Multazim({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3')
    });

    test('blank env variable', () => {
      process.env['MULTAZIM_BASE_URL'] = '  '; // blank
      const client = new Multazim({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3')
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Multazim({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['MULTAZIM_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Multazim({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Multazim({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Multazim({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Multazim({ apiKey: 'My API Key', fetch: testFetch });

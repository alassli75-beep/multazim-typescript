...
import { Category, Pet, PetCreateParams, PetFindByStatusParams, PetFindByStatusResponse, PetFindByTagsParams, PetFindByTagsResponse, PetUpdateByIDParams, PetUpdateParams, PetUploadImageParams, PetUploadImageResponse, Pets, Tag } from './resources/pets';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['MULTAZIM_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['MULTAZIM_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Multazim API. 
 */
export class Multazim {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['MULTAZIM_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('MULTAZIM_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.MultazimError(
        'The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Multazim client with an apiKey option, like new Multazim({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.timeout = options.timeout ?? Multazim.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('MULTAZIM_LOG'), 'process.env[\'MULTAZIM_LOG\']', this) ?? defaultLogLevel;
  ...
  static Multazim = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static MultazimError = Errors.MultazimError;
...
Multazim.Pets = Pets;
Multazim.Store = Store;
Multazim.Users = Users;

export declare namespace Multazim {
